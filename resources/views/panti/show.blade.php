            ...
            <div class="flex flex-col md:flex-row gap-8">
                ...
                <div class="md:w-2/3">
                    ...
                    <div class="flex gap-4">
                        <button onclick="openDonationModal{{ $panti->id_panti }}()" class="flex-1 bg-[#41644A] hover:bg-[#0D4715] text-[#F1F0E9] font-bold py-3 px-6 rounded-lg text-center flex items-center justify-center transition-colors duration-300">
                        ...
                        @auth
                        <button id="bookmarkBtn{{ $panti->id_panti }}" onclick="toggleBookmark({{ $panti->id_panti }})" class="bg-[#E9762B] hover:bg-[#0D4715] text-white font-bold py-3 px-4 rounded-lg text-center flex items-center justify-center transition-colors duration-300">
                            <svg id="bookmarkIcon{{ $panti->id_panti }}" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
{{-- Donation Modal --}}
@include('components.donation-modal', ['panti' => $panti, 'modalId' => 'donationModal' . $panti->id_panti])

@auth
<script>
// Cek status bookmark saat halaman dimuat
document.addEventListener('DOMContentLoaded', function() {
    checkBookmarkStatus({{ $panti->id_panti }});
});

function checkBookmarkStatus(pantiId) {
    fetch(`/bookmark/panti/${pantiId}/check`)
        .then(response => response.json())
        .then(data => {
            updateBookmarkButton(pantiId, data.isBookmarked);
        })
        .catch(error => {
...
function toggleBookmark(pantiId) {
    fetch(`/bookmark/panti/${pantiId}/toggle`, {
        method: 'POST',
        headers: {
...
function updateBookmarkButton(pantiId, isBookmarked) {
    const button = document.getElementById(`bookmarkBtn${pantiId}`);
    const icon = document.getElementById(`bookmarkIcon${pantiId}`);
