...
@section('content')
<section class="relative bg-gradient-to-br from-primary-cream via-light-cream to-soft-gray py-20 -mt-32">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
            @forelse($news as $index => $article)
                ...
                >
                        <a href="/berita/{{ $article->id_artikel }}" class="block news-card" style="animation-delay: {{ $index * 0.1 }};">
                        <div class="bg-white rounded-3xl shadow-lg hover:shadow-xl transition-all duration-500 p-6 border border-[#D0D5CB] h-full relative overflow-hidden group">
                            ...
                            <div class="relative z-10">
                                <img class="w-full h-48 rounded-2xl shadow-md object-cover mb-6 group-hover:scale-105 transition-transform duration-300" src="{{ asset('storage/' . $article->gambar) }}" alt="Artikel" loading="lazy" width="800" height="600">
                                ...
                                <div class="flex items-center gap-3 mb-4">
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center justify-between">
                                            <p class="text-sm text-[#E9762B] font-bold">{{ $article->author->name }}</p>
                                            <p class="text-sm text-[#41644A] font-bold">{{ $article->kategori->nama }}</p>
                                        </div>
                                        <p class="text-xs text-[#41644A]">{{ $article->publish_date->diffForHumans() }}</p>
                                    </div>
                                </div>
                                
                                <h3 class="text-xl font-bold mb-4 text-[#0D4715] group-hover:text-[#E9762B] transition-colors duration-300">{{ $article->judul }}</h3>
