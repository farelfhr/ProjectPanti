...
@section('content')
    ...
    <div class="bg-white p-8 rounded-lg shadow-lg mb-8">
        ...
        <form action="{{ route('admin.panti.update', $panti) }}" method="POST">
            @method('PUT')
        ...
        <form action="{{ route('admin.kebutuhan.store') }}" method="POST" class="mb-6 p-4 border rounded-md">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="tipe" id="tipe" placeholder="Cth: Sembako, Dana, Pakaian" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                </div>
                <div>
                    <label for="deskripsi" class="block text-sm font-medium text-gray-700">Deskripsi</label>
                    <input type="text" name="deskripsi" id="deskripsi" placeholder="Cth: Beras, Gula, Baju layak pakai" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                </div>
                <div>
                    <label for="urgency_level" class="block text-sm font-medium text-gray-700">Tingkat Urgensi</label>
                    <select name="urgency_level" id="urgency_level" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
            ...
            <button type="submit" class="mt-4 inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Tambah Kebutuhan</button>
        </form>
            ...
            <tbody class="text-gray-700">
                @forelse ($panti->kebutuhan as $kebutuhan)
                    <tr class="border-b">
                        ...
                        <td class="py-2 px-3">
                             <form action="{{ route('admin.kebutuhan.destroy', $kebutuhan) }}" method="POST" onsubmit="return confirm('Yakin hapus kebutuhan ini?');">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr><td colspan="4" class="text-center py-4">Panti ini belum memiliki daftar kebutuhan.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
